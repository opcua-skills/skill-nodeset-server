...
#ifndef GRASP_SKILL_HPP
#define GRASP_SKILL_HPP
...
namespace skill {
    ...
    class GraspSkill : virtual public base::SkillBase {
    ...
    public:
        ...
        UA_StatusCode startMethod(
            ...
            double force = *(UA_Double*)input[1].data;

            UA_LOG_INFO(server->getLogger(), UA_LOGCATEGORY_USERLAND,
                        "--> GraspSkill - Start: span = %f, force = %f", span, force);
            ...
            std::thread t = std::thread([this, span, force]() {
        ...
        void grasp(double span, double force) {
            // just sleep for a second. Normally you would here now control the gripper
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }


        bool graspWithTransitions(double span, double force) {
